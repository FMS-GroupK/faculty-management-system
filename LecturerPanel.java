...
public class LecturerPanel extends JPanel {
    ...
    private void openForm(Lecturer existing) {
        ...
        JComboBox<Course> coursesCombo = new JComboBox<>(courses.toArray(new Course[0]));
        ...
        if (existing != null && existing.getCoursesTeaching() != null) {
            for (Course c : courses) {
                if (c.getCourseName().equals(existing.getCoursesTeaching())) {
                    coursesCombo.setSelectedItem(c);
                    break;
                }
            }
        }
        ...
        form.add(deptCombo);
        form.add(new JLabel("Courses Teaching:"));
        form.add(coursesCombo);
        ...
        if (result == JOptionPane.OK_OPTION) {
            ...
            Course selectedCourse = (Course) coursesCombo.getSelectedItem();
            String coursesTeaching = selectedCourse != null ? selectedCourse.getCourseName() : "";

            if (existing == null) {
                Lecturer l = new Lecturer(0, nameField.getText().trim(), deptId,
                        emailField.getText().trim(), mobileField.getText().trim());
                l.setCoursesTeaching(coursesTeaching);
                controller.addLecturer(l);
            } else {
                existing.setFullName(nameField.getText().trim());
                existing.setDepartmentId(deptId);
                existing.setCoursesTeaching(coursesTeaching);
